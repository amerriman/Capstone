app.directive('writingAnalysisSample', function() {
  return {
    restrict: 'E',
    templateUrl: '/writing-analysis/writing-analysis-sample.html',
    controller: ['$rootScope', '$scope', '$http', '$auth', '$location', '$route', '$window', 'httpFactory', function ($rootScope, $scope, $http, $auth, $location, $route, $window, httpFactory){
      ...
      appendText = function(text) {
        var myEl = angular.element( document.querySelector( '#results-here' ) );
        myEl.append(text);
      };

      //removes the analyzed paragraph from the DOM
      $scope.unappendText = function() {
